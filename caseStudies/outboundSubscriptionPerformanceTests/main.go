...
func sendRequestToOutboundSubscription(endpoint string, body []byte) {
...
func (p *PerformanceTestRun) timeEvent(worker int) {
	...
	p.mu.Unlock()
	sendRequestToOutboundSubscription(p.test.oracleEndpoint, jsonStr)
...
func main() {
	...
	test := &PerformanceTest{
		...
		oracleEndpoint:  "http://localhost:8080/smartContractProviders/2/events",
