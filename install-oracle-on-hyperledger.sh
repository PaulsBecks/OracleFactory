...
peer lifecycle chaincode install ../../hyperledgerOnChainOracle/$CHAINCODE_NAME.tar.gz
export CC_PACKAGE_ID=$(peer lifecycle chaincode queryinstalled | grep "Package ID: on-chain-oracle" | sed -E 's/.*(on-chain-oracle:[0-9a-f]+),.*/\1/') 
echo $CC_PACKAGE_ID
